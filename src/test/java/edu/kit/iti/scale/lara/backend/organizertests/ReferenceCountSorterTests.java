...
import edu.kit.iti.scale.lara.backend.model.organizer.sorting.ReferenceCountSorter;
...
@InMemoryTest
public class ReferenceCountSorterTests {
    @Autowired
    AuthorRepository authorRepository;

    private final Organizer<Paper> referenceCountSorter = new ReferenceCountSorter<>("ascending");
    ...
    public void testCitationCountSorter() {
        ...
        List<Paper> sortedPapers = referenceCountSorter.organize(papers);

        for (int i = 1; i < sortedPapers.size(); i++)
            Assertions.assertThat(sortedPapers.get(i).getReferenceCount()).isGreaterThanOrEqualTo
                    (sortedPapers.get(i - 1).getReferenceCount());
    }
